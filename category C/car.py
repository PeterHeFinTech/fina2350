...
def run_event_study():
    print("Status: Initializing Event Study with Robust Aggregation...")
    ...
    try:
        ...
        # First aggregation: remove duplicates by publish_date
        news = news.loc[news.groupby('publish_date')['nvda_relevance'].idxmax()].reset_index(drop=True)
        print(f"After first aggregation: {len(news)} unique publish dates")
    ...
    # D. Output Generation with Final Deduplication
    if results:
        output_df = pd.DataFrame(results)
        
        # Final aggregation: ensure no duplicate event_date
        output_df = output_df.loc[output_df.groupby('event_date')['nvda_relevance'].idxmax()].reset_index(drop=True)
        
        output_df.to_csv(OUT_FILE, index=False)
        print(f"Success! Filtered out {skipped_count} low-relevance items.")
        print(f"Final dataset after deduplication: {len(output_df)} events saved to {OUT_FILE.name}")
    else:
